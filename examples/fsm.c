...
void a_task(void* args) {
    ...
    for (;;) {
        int32_t event = ss_fsm_event_receive(NULL);
...
void main_task(void* args) {
    ...
    for (;;) {
        int32_t event = ss_fsm_event_receive_core();
...
int main(void) {
    ...
    ss_printf(4, "fse_pb_bsp example: fsm \r\n");

    SS_ERROR_ASSERT(ss_rtos_task_add(a_task, NULL, 1, "a_task"));
    SS_ERROR_ASSERT(ss_rtos_task_add(b_task, NULL, 1, "b_task"));
    SS_ERROR_ASSERT(ss_rtos_task_add(main_task, NULL, 1, "main_task"));
